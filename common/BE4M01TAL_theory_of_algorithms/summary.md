...
⭐ **Příklad: dynamické pole** (Čmolík 2013, Q33 — chtěl právě tento): pole se při zaplnění **zdvojnásobí** (realokace + překopírování). Jedno vložení stojí v nejhorším případě $\Theta(n)$, ale posloupnost $n$ vložení stojí $n + (1 + 2 + 4 + \dots + 2^{\lfloor\lg n\rfloor}) < n + 2n = \Theta(n)$ ⇒ **amortizovaně $O(1)$** na vložení. Účetní metoda: každé vložení zaplatí 3 kredity (1 za sebe, 2 předplatné na budoucí kopii sebe a jednoho staršího prvku). Pointa pro odpověď: **asymptotická složitost** popisuje růst funkce; **amortizovaná** je záruka pro průměr přes nejhorší *posloupnost* operací — bez pravděpodobnosti.
...
### 2.8 ⭐ Přehled modelů TM a jejich ekvivalence (Blaškova otázka 2023, Q2)

*„Popište DTM, NTM a alespoň tři další modely; diskutujte ekvivalenci a časovou/paměťovou náročnost."* Všechny modely přijímají **tutéž třídu jazyků (RS)** — liší se jen efektivitou:

| Model | Co přidává | Zpomalení při simulaci na 1-páskovém DTM |
|---|---|---|
| **Vícepáskový TM** | $k$ nezávislých pásek | **kvadratické**: $T(n) \to O(T(n)^2)$ (§2.5) |
| **NTM** | volba kroku z množiny | **exponenciální**: prohledávání stromu výpočtů (§2.6) |
| **RTM** | read-only páska náhodných bitů | deterministicky: vyzkoušení všech posloupností bitů — exponenciální; jako výpočetní model ekvivalentní (§5.1) |
| **Univerzální TM** | interpretuje kód $\langle M\rangle$ | simulace s polynomiálním zpomalením (§6.4) |
| **RAM** | přímý přístup do paměti, aritmetika | TM simuluje RAM v $O(n^3)$ (vícepáskový) až $O(n^6)$ (1-páskový); RAM simuluje TM v $O(n^2)$ (§2.7) |
| **Oboustranně vs. jednostranně nekonečná páska** | — | konstantní faktor (přeložení pásky na 2 stopy) |

Pointa k vyslovení: **výpočetní síla je stejná** (Churchova-Turingova teze), polynomiální převoditelnost mezi deterministickými modely činí třídu P **robustní**; jediný model, kde polynomiální ekvivalence není známa, je NTM (= otázka P vs NP).
...
**⭐⭐ Tranzitivita s důkazem (*doh §4.3.2*)** — Tišer ji chtěl dokázat 2× (2017, 2018), umět zpaměti:

> $U \propto_p V \land V \propto_p W \Rightarrow U \propto_p W$.
> **Důkaz.** Nechť $A$ redukuje $U$ na $V$ v čase $p(n)$ a $A'$ redukuje $V$ na $W$ v čase $q(n)$, kde $p, q$ jsou polynomy. Složený algoritmus $A'(A(\cdot))$ převádí instance $U$ na instance $W$ se zachováním odpovědi (složení dvou ekvivalencí). Složitost: $A$ běží $p(n)$ kroků, takže jeho **výstup má délku nejvýše $p(n)$** (za 1 krok napíše max 1 symbol — tohle je ta jemnost, kterou Tišer doloval!); $A'$ na něm běží $\le q(p(n))$ kroků. Celkem $p(n) + q(p(n))$ — složení polynomů je polynom. $\blacksquare$

**⭐⭐ Je $\propto_p$ uspořádání?** — Hekrdlova oblíbená otázka, položil ji 3× (2014×2, 2015):

- **Reflexivní** ✓ (identické zobrazení je polynomiální redukce $U \propto_p U$).
- **Tranzitivní** ✓ (důkaz výše).
- **Antisymetrická** ✗ — $U \propto_p V \land V \propto_p U \not\Rightarrow U = V$ (dvě vzájemně převoditelné úlohy nejsou táž úloha; např. klika a nezávislá množina). ⇒ $\propto_p$ je jen **kvaziuspořádání (preorder)**, nikoli uspořádání.
- **Jak z toho uspořádání udělat** (na to Hekrdla naváděl): přejít na **třídy ekvivalence** vzájemně převoditelných úloh ($U \sim V \Leftrightarrow U \propto_p V \land V \propto_p U$); na nich už $\propto_p$ indukuje částečné uspořádání. **NPC je v tomto uspořádání maximum třídy NP** — přesně tahle odpověď mu stačila na A.
...
⭐ **Příklad NP-těžké úlohy, která není NPC** (chtěno 2012, Q28): (a) **halting problém** — je NP-těžký (SAT na něj lze redukovat), ale není ani rozhodnutelný, natož v NP; (b) **optimalizační verze TSP** — není rozhodovací úloha, takže formálně nemůže být v NP, ale rozhodovací TSP se na ni triviálně redukuje.

⭐ **Hekrdlovy řetězené dotazy (2014, Q21):** *„Co víme o úloze, na kterou se redukuje nějaká NPC úloha?"* → je **NP-těžká** (nic víc — nemusí být v NP). *„Co kdybychom našli DTM řešící NP-těžkou úlohu v polynomiálním čase?"* → každá úloha z NP se na ni redukuje ⇒ **P = NP**.
...
**Složitost:** přidáme max $(s-3)k$ nových proměnných a prodloužíme formuli o max $2(s-3)k$ literálů ⇒ polynomiální. (Žukovec se 2023 doptávala právě na zdůvodnění polynomiality — odpověď: počet nových proměnných i klauzulí je lineární v délce vstupní formule.)

⭐ **Konkrétní instance** (Tišer 2014 ji nechal převést na místě): klauzule $(a \lor b \lor c \lor d)$ má $s = 4$ literály ⇒ jedna nová proměnná $x_1$:
$$(a \lor b \lor c \lor d) \;\rightsquigarrow\; (a \lor b \lor x_1) \land (\lnot x_1 \lor c \lor d).$$
Kontrola: je-li některý z $a,b,c,d$ pravdivý, lze $x_1$ dovolit; nejsou-li, první klauzule nutí $x_1 = 1$ a druhá pak selže. Klauzule s 1–2 literály doplníme na přesně 3: $(l_1 \lor l_2) \rightsquigarrow (l_1 \lor l_2 \lor y) \land (l_1 \lor l_2 \lor \lnot y)$; jednoliterálovou analogicky se dvěma dummy proměnnými (4 klauzule se všemi kombinacemi polarit) — obě polarity zaručují, že dummy proměnné splnitelnost neovlivní.
...
### 3.7.17 ⭐⭐ Klasifikační drily — „do které třídy patří…?"

Demlová a Žukovec téměř vždy zakončují konkrétní úlohou k zařazení. Vzorové odpovědi (vždy: zformulovat rozhodovací verzi → zařadit → zdůvodnit certifikátem/algoritmem/redukcí):

| Úloha | Třída | Zdůvodnění |
|---|---|---|
| **4-barevnost** (Žukovec 2022) | NP, **NPC** | ∈ NP: certifikát = obarvení, ověření hran v $O(m)$ **nedeterministickým algoritmem** (to chtěla slyšet!); NPC: redukce 3-color $\propto_p$ 4-color přidáním vrcholu spojeného se všemi (§3.7.15). |
| **Nejkratší cesta** (Žukovec 2018) | **P** | Dijkstra/Bellman-Ford — bez záporných cyklů. **Jednoduchá** nejkratší cesta v grafu **se zápornými cykly** je NPC (§3.7.14). |
| **„∃ nejkratší cesta délky ≥ k?"** (Demlová 2014, chyták!) | **P** | Nejde o nejdelší cestu: spočti délku nejkratší cesty $d$ Bellman-Fordem a porovnej $d \ge k$. Chyták zní jako longest path (NPC), ale ptá se na vlastnost *nejkratší* cesty. |
| **Úloha mimo NP** (Žukovec 2018) | — | Bezpečná odpověď: **nerozhodnutelné úlohy** — PCP, halting problém ($L_{UN}$). Nerozhodnutelná úloha nemůže být v NP (NP ⊆ R přes exponenciální simulaci NTM). |
| **Toky $\propto_p$ batoh?** (Demlová 2017) | ano | Toky ∈ P ⊆ NP a batoh je NPC ⇒ **každá** úloha z NP se na batoh polynomiálně redukuje. Pozor na směr! A toky **nejsou** NP-těžké (jsou v P; ta záměna stála studenta body). |
| **TSP, batoh, toky — zařadit** (Demlová 2017) | NPC, NPC, P | TSP/batoh: rozhodovací verze NPC (§3.7.12, §3.7.6); maximální tok: Ford-Fulkerson/Dinic v P. |
| **„TM přijímá slovo 00"** (Demlová 2023) | $\mathbf{RS} \setminus \mathbf{R}$ | $L = \{\langle M\rangle \mid 00 \in L(M)\}$: netriviální vlastnost RS jazyků ⇒ **nerozhodnutelná** (Riceova věta §6.6); v RS je (simuluj $M$ na 00, přijmi při přijetí). Student řekl „P" — proto „lepší D". |

**Alternativní notace** (Rogalewicz 2014 ji vyžadoval): $\mathbf{P} = \bigcup_k \mathrm{DTIME}(n^k)$, $\mathbf{NP} = \bigcup_k \mathrm{NTIME}(n^k)$, kde DTIME/NTIME$(f(n))$ = jazyky rozhodnutelné D/N-TM v čase $O(f(n))$. Stejný pojem, jiný zápis — nenechat se rozhodit.
...
### 5.9 Bonusové otázky z archivu

- ⭐ **„Navrhněte pravděpodobnostní algoritmus pro výpočet čísla π"** (Hekrdla 2014, Q20): **Monte Carlo „terč"** — generuj $N$ náhodných bodů $(x, y) \in [0,1]^2$; podíl bodů s $x^2 + y^2 \le 1$ konverguje k $\pi/4$ (obsah čtvrtkruhu / obsah čtverce). Odhad $\hat\pi = 4 \cdot \#\text{uvnitř}/N$; chyba $O(1/\sqrt N)$ (CLT). Ilustruje Monte Carlo metodu mimo rozhodovací úlohy — výsledek je vždy „jen" odhad, na rozdíl od Las Vegas.
- **Pozn. k terminologii:** obecná třída se zaručenou pravděpodobností chyby na obou stranách je **BPP** ($\Pr[\text{správně}] \ge 2/3$ pro obě odpovědi); v Demlové kurzu se neprobírá do hloubky, ale hodí se vědět, že RP ⊆ BPP a otázka BPP vs P je otevřená.
...
⭐ **Doplňky chtěné u zkoušky:**

- **Rozhoduje-li jazyk NTM, je rekurzivní?** (Žukovec 2018, Q32) — **Ano.** NTM rozhodující $L$ (každá větev se zastaví) lze determinizovat (§2.6); deterministická simulace prohledá strom výpočtů konečné hloubky, vždy se zastaví ⇒ $L \in \mathbf{R}$. (Definice R typ stroje nespecifikuje — determinismus sílu nemění, jen čas.)
- **Zařazení do Chomského hierarchie** (Rogalewicz 2022, Q4): **RS = typ 0** (jazyky generované neomezenými gramatikami = jazyky přijímané TM). **R nemá gramatickou charakterizaci** — platí typ 1 (kontextové) $\subsetneq$ **R** $\subsetneq$ RS = typ 0. Celý řetěz: typ 3 (regulární, KA) $\subsetneq$ typ 2 (bezkontextové, zásobníkové automaty) $\subsetneq$ typ 1 (kontextové, LBA) $\subsetneq$ R $\subsetneq$ RS.
- **Halting problém** $L_{halt} = \{\langle M\rangle w \mid M \text{ se na } w \text{ zastaví}\}$ — v RS (simuluj; zastaví-li se, přijmi), není v R (redukce z $L_{UN}$, příp. přímý diagonální argument). U státnice často zaměňováno s $L_{UN}$ — obojí je korektní příklad jazyka v $\mathbf{RS} \setminus \mathbf{R}$.
