...
- **LDA** — Gaussians with a **common covariance `Σ`** for all classes ⇒ **linear** discriminant.
  ...
  - ⭐ **Why the boundary is linear — the 3-line derivation** (deliver this when Kléma says "vysvětlete rozhodování"): classify by the largest `log(πₖ fₖ(x))`. Expanding the Gaussian: `log πₖ − ½(x−μₖ)ᵀΣ⁻¹(x−μₖ) + const`. The quadratic term `−½xᵀΣ⁻¹x` is **the same for every class** (shared `Σ`) ⇒ it cancels when comparing classes, leaving the linear `δₖ(x)`. Decision boundary between classes `k,l` = the hyperplane `δₖ(x)=δₗ(x)`.
  - **Parameters learned** (`learning = counting + averaging`, closed form, no iteration): `π̂ₖ` = class frequencies, `μ̂ₖ` = class means, `Σ̂` = pooled within-class covariance. Count: `(K−1) + Kp + p(p+1)/2`.
- **QDA** — Gaussians with **class-specific `Σₖ`** ⇒ the `xᵀΣₖ⁻¹x` terms **no longer cancel** ⇒ **quadratic** boundary `δₖ(x) = −½ log|Σₖ| − ½(x−μₖ)ᵀΣₖ⁻¹(x−μₖ) + log πₖ`. More flexible (lower bias) but `K·p(p+1)/2` covariance parameters instead of one shared set (higher variance) — needs much more data. **LDA vs QDA is a pure bias–variance choice** controlled by whether the equal-covariance assumption is plausible and how large `n` is.
...
**Derivation A — maximum variance (the one to reproduce at the board).** Find the direction `u`, `‖u‖=1`, maximizing the variance of the projected data. Projected variance = `(1/m)Σᵢ(uᵀxᵢ)² = uᵀC_X u`. Maximize with the Lagrangian `uᵀC_X u − λ(uᵀu−1)`; setting the gradient to zero gives
```
C_X u = λu
```
— `u` must be an **eigenvector** of the covariance matrix, and the achieved variance is `uᵀC_X u = λ` ⇒ take the eigenvector with the **largest eigenvalue**; further components = next eigenvectors (orthogonality constraint). So *"directions of maximal variance = top eigenvectors, eigenvalue = variance captured"* falls straight out of one Lagrange multiplier.

**Derivation B — decorrelation view** (zero-centred `X`, `Σ xᵢ=0`):
...
4. **Reduction** `T_{m×L}=X_{m×D}P_{D×L}` (keep `L` largest-eigenvalue components); **reconstruction** `X̂=T Pᵀ`. Dropping small components = controlled information loss; equivalently PCA **minimizes reconstruction error** among all linear rank-`L` maps (the third equivalent view).
- **Practical**: center always; **standardize** when features have different units (else high-variance features dominate); choose `L` by cumulative explained variance `Σλᵢ/Σλ` (e.g. 90–95%) or scree-plot elbow.
...
- ⭐ **Why it converges but not necessarily optimally** (Mrázová wanted exactly this): both steps can only **decrease** `W` — the assign step moves each point to its nearest centroid (each summand can only drop), the update step replaces each centroid by the mean, which *minimizes* the within-cluster sum of squares for fixed assignments. `W ≥ 0` is monotonically non-increasing and there are **finitely many partitions** (Stirling, §6.1) ⇒ it must terminate. But it's coordinate descent on a non-convex objective ⇒ only a **local** optimum; a bad init can lock in a bad partition (hence multiple restarts / **k-means++**, which spreads initial centroids with probability ∝ squared distance).
...
**EM for GMM**: `P(xᵢ|θ)=Σⱼ αⱼ·N(xᵢ; μⱼ, Σⱼ)`, parameters `θ={αⱼ, μⱼ, Σⱼ}` with `Σαⱼ=1`. Direct MLE is intractable (log of a sum); EM iterates two closed-form steps:

- **E-step** — compute **responsibilities** (posterior membership of each point in each cluster) with current `θ`:
```
γᵢⱼ = P(Cⱼ|xᵢ) = αⱼ·N(xᵢ; μⱼ,Σⱼ) / Σₗ αₗ·N(xᵢ; μₗ,Σₗ)        (Bayes)
```
- **M-step** — re-estimate parameters as **responsibility-weighted MLE** (let `mⱼ = Σᵢ γᵢⱼ` = effective cluster size):
```
μⱼ = (1/mⱼ) Σᵢ γᵢⱼ xᵢ
Σⱼ = (1/mⱼ) Σᵢ γᵢⱼ (xᵢ−μⱼ)(xᵢ−μⱼ)ᵀ
αⱼ = mⱼ / m
```
— exactly the usual mean/covariance/frequency formulas, with each point counted *fractionally* per its responsibility.

- ⭐ **Why EM works** (the follow-up to expect; full derivation in SSU summary §EM): each iteration maximizes a **lower bound** on the log-likelihood (Jensen's inequality); E-step makes the bound tight at current `θ`, M-step maximizes it ⇒ **likelihood never decreases** ⇒ converges (to a local maximum/saddle — initialization matters, like k-means).
- ⭐ **k-means = degenerate EM for GMM**: fix `Σⱼ = σ²I` with `σ²→0` and `αⱼ=1/k` — responsibilities collapse to hard 0/1 assignment to the nearest mean (E-step = assign) and the weighted mean becomes the plain mean (M-step = update). Hence: k-means assumes **spherical, equally-sized clusters**; GMM relaxes this to **elliptical** ones via `Σⱼ`.
- **Soft clustering**: membership is a probability (`Σⱼ γᵢⱼ=1`); assign to max-posterior cluster if a partition is needed. More flexible than k-means, **slower**; `k` via likelihood + AIC/BIC (penalized — likelihood alone always grows with `k`).
